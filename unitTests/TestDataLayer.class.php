<?php
	...
	require_once(dirname(__FILE__) . "/../backend/dataLayer/DataLayer.class.php");
	...
	class TestDataLayer extends UnitTestCase{
		...
		function testChats(){
			...
			$database = DataLayer::getPlayerInstance();
			$chatDatabase = DataLayer::getChatInstance();
			...
			$chatDatabase->postChat($joshArr["id"], $samArr["id"], $joshMessage);
			$chatDatabase->postChat($samArr["id"], $joshArr["id"], $samMessage);
			
			
			$chats1 = $chatDatabase->getChats($joshArr["id"], $samArr["id"]);

			$this->assertTrue($chats1[0]["posterID"], $joshArr["id"]);
			$this->assertTrue($chats1[0]["content"], $joshMessage);
			
			$this->assertTrue($chats1[1]["posterID"], $samArr["id"]);
